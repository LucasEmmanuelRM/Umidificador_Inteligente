...
void ssd1306_draw_number(ssd1306_t *ssd, int num, int x, int y) {
  if (num < 0 || num > 999) return; // Garantir que o número esteja entre 0 e 999

  char digito1 = '0' + (num / 100);      // Primeiro dígito (centenas)
  char digito2 = '0' + ((num / 10) % 10); // Segundo dígito (dezenas)
  char digito3 = '0' + (num % 10);       // Terceiro dígito (unidades)

  ssd1306_draw_char(ssd, digito1, x, y);      // Desenha o primeiro dígito na posição x, y
  ssd1306_draw_char(ssd, digito2, x + 8, y);  // Desenha o segundo dígito deslocado 8 pixels à direita
  ssd1306_draw_char(ssd, digito3, x + 16, y); // Desenha o terceiro dígito deslocado 16 pixels à direita
}
